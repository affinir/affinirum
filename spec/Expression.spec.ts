...
describe('Expression Evaluation test', ()=> {
	[
		...
		[ '[a, b, c]->add([ 1, 2, 3, 4 ])->reduce(num(num acc,num val)(acc+val))', [ { a: 1, b: 2, c: 3, result: 16 } ] ],
		...
		[ 'var a:[0,10,200,3000,40000], a->keys()[1] + a->values()[2]', [ { result: 201 } ] ],
		[ '[100,200,300]->entries()[1][1]+[1,2,3]->entries()[1][0]', [ { result: 201 } ] ],
		[ '[0,1,2,3]->add([10,20,30,40])[5]', [ { result: 20 } ] ],
		[ '[1,2,3,4]->reduce(num (num a, num b) a->subtract(b))', [ { result: -8 } ] ],
		[ 'a->unique()->reduce(var (var acc,var val)(acc+val))', [ { a: [ 1, 2, 3, 3, 2, 1 ], result: 6 }, { a: [ 'a', 'b', 'c', 'a', 'b', 'c' ], result: 'abc' } ] ],
		...
		[ 'arr0->map(number(num val)(val*2*t))->filter(boolean(number val)(val>5))[1]+[9]->length()', [ { arr0: [ 1, 2, 3 ], t: 2, result: 13 } ] ],
		...
		[ 'a->add(b,c).i = 0', [ { a: { f: 0, d: 0 }, b: { i: 0 }, c: {}, result: true }, { a: { i: 50, d: 0 }, b: { i: 0 }, c: { prop: 1 }, result: true } ] ],
		[ 'compose([a, 10],[`b`, `my string`])."a"', [ { a: 'a', result: 10 }, { a: 'b', result: undefined } ] ],
		...
		[ 'func f:bool(num a) (a:a*100, a>0),a->filter(f)->sum()', [ { a: [ -10, -20, 1, 2 ], result: 3 } ] ],
