...
describe('Expression Evaluation test', ()=> {
	[
		...
		[ '"0"+"1"+`2`', [ { result: '012' } ] ],
		[ '(x + 10 + \\v0) * (y - 10)>0', [ { x: 10, y: 10, result: false }, { x: 100, y: 100, result: true } ] ],
		...
		[ 'join([0,1,2,3,4], ":")', [ { result: '0:1:2:3:4' } ] ],
		[ 'union([0,1,2,3], [10,20,30,40])[5]', [ { result: 20 } ] ],
		[ 'add([0,1,2,3]#[10,20,30,40]#100)', [ { result: 206 } ] ],
		...
		[ 'arr0.map(array(array a)(a.map(number(number b)(b+12)))).add()', [ { arr0: [ [ 1 ], [ 1, 2 ], [ 2, 3, 4 ] ], result: 85 } ] ],
		...
		[ 'merge([a,b],c).i = 0', [ { a: { f: 0, d: 0 }, b: { i: 0 }, c: {}, result: true }, { a: { i: 50, d: 0 }, b: { i: 0 }, c: { prop: 1 }, result: true } ] ],
		...
		[ 'func f:bool (num a) (a>0),a.filter(f).add()', [ { a: [ -10, -20, 1, 2 ], result: 3 } ] ],
	].forEach(([ expr, args ])=> {
