	...
	([
		...
		['AN.Format(obj0)', [
			...
			{ obj0: {
				...
				xarr: [1n, 2n, 3n],
				xobj: { a: 1, b: 2 },
			}, result: '["xbool":true,"xfloat":-50.0,"xbuf":#0a141e0004430c0bc8dc0032,"xstr":"string value","xfunc":function,"xarr":[1,2,3],"xobj":["a":1.0,"b":2.0]]'},
		]],
		['_var', [
		...
		['-a^2 == b', [{ a: 2, b: -4, result: true }, { a: 2n, b: -5n, result: false }]],
		['_1.Power(2)+40 % 25', [{ _1: 10, result: 115 }, { _1: 100n, result: 10015n }]],
		...
		['Boolean.Decode(v.Encode())', [{ v: true, result: true }, { v: false, result: false }]],
		['Timestamp.Decode(Timestamp.Epoch(v).Encode).EpochTime', [{ v: 60n, result: 60n }, { v: 1055n, result: 1055n }]],
		['Float.Decode(v.Encode(enc), enc)', [{ v: 50, enc: 'float32le', result: 50 }, { v: 10, enc: 'float64', result: 10 }]],
		['Integer.Decode(v.Encode(enc), enc)', [{ v: 0n, enc: 'uint16', result: 0n }, { v: 1055n, enc: 'uint32', result: 1055n }, { v: 1055n, enc: 'uint16le', result: 1055n }]],
		['String.Decode(v.Encode(enc), enc)', [{ v: '', enc: 'utf8', result: '' }, { v: '1055', enc: 'utf8', result: '1055' }, { v: '1055', enc: 'ucs2le', result: '1055' }]],
		['a.Byte(b).Format()', [{ a: new Uint8Array([0xff, 0xff, 0x00, 0x01]).buffer, b: 3n, result: '01' }, { a: new Uint8Array([0x10, 0x00]).buffer, b: 0n, result: '10' }]],
		...
		['String.Alphanum(a)', [{ a: '---abcd===', result: 'abcd' }, { a: '+abc-def!', result: 'abcdef' }]],
		['a.Splice(1, 1, b)', [{ a: 'abcdef', b: '12', result: 'a12cdef' }]],
		['a.Inject(1, b)', [{ a: 'abcdef', b: '12', result: 'a12bcdef' }]],
		['a.Splice(1, 1, b).Format', [{ a: 'abcdef'.split(''), b: '12'.split(''), result: 'a12cdef' }]],
		['a.Inject(1, b).Format', [{ a: 'abcdef'.split(''), b: '12'.split(''), result: 'a12bcdef' }]],
		...
		['[20,21,22,23,24].Format(f, sep)', [{ f: undefined, sep: undefined, result: '2021222324' }, { f: undefined, sep: ':', result: '20:21:22:23:24' }, { f: 16n, sep: ':', result: '14:15:16:17:18' }]],
		...
		['Timestamp.Epoch(a.EpochTime).Format(6)', [{ a: new Date('2020-02-25 20:30:45.555Z'), result: '45' }]],
		['a.Format(1)+a.Format(2) + a.Format(3)+a.Format(4)+a.Format(5)+a.Format(6)+a.Format(7)', [{ a: new Date('2020-02-20 20:30:40.555Z'), result: '20200220203040555' }]],
