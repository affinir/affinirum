...
describe('Evaluation Result test', ()=> {
	([
		...
		['var s:integer=0;Array.Range(start, end).Mutate(~void(x:number){s=s+x});s', [{ start: 1n, end: 11n, result: 55n }, { start: -1n, end: -11n, result: -65n }]],
		['[p,11].Mutate(~ integer?(a:integer) {const t=10; if (a>10){t}else{null} } )[i]', [{ i: 1n, p: 10n, result: 10n }, { i: 0n, p: 1n, result: undefined }]],
		['[p,p,1, 2, 3].Mutate(~number?(a:number?){a?:10})[1]', [{ p: undefined, result: 10n }, { p: 0, result: 0 }]],
		['arr1.First(~boolean(v:number, i:integer){v==2})', [{ arr1: [1, 2, 3], result: 2 }, { arr1: [2, 2, 3], result: 2 }]],
		['arr1.Last(~boolean(v:number, i:integer){(i==1)})', [{ arr1: [1, 2, 3], result: 2 }, { arr1: [10, 20, 30], result: 20 }]],
		['arr1.FirstIndex(~boolean(v:number, i:integer){(v==2)})', [{ arr1: [1, 2, 3], result: 1 }]],
		['arr1.LastIndex(~boolean(v:number, i:integer){i==1})', [{ arr1: [1, 2, 3], result: 1 }]],
		['arr1.Filter(~boolean(v:number, i:integer, a:array){(a[i]*i>2)})[1]', [{ arr1: [1, 1, 5, 4, 1], result: 4 }]],
		['arr0.Mutate(~number(val:number){val*2*t}).Filter(~boolean(val:number){val>5})[1]+[9].Length()', [{ arr0: [1, 2, 3], t: 2, result: 13 }]],
		['Number.Sum(arr0.Mutate( ~ array(a:array){ a.Mutate(~number(b:number){b+12.0})} ).Flatten())', [{ arr0: [[1], [1, 2], [2, 3, 4]], result: 85 }]],
