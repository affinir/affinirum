...
describe('Expression Evaluation Result test', ()=> {
	([
		...
		['4.1e-1', [{ result: 0.41 }]],
		['4.1e-01', [{ result: 0.41 }]],
		['#0.FormatBuffer', [{ result: '00' }]],
		['#ffff.FormatBuffer', [{ result: 'ffff' }]],
		['#AAAA.FormatBuffer', [{ result: 'aaaa' }]],
		['#.FormatBuffer', [{ result: '' }]],
		['a.FormatBuffer', [{ a: new Uint8Array().buffer, result: '' }]],
		['""', [{ result: '' }]],
		['"0 "', [{ result: '0 ' }]],
		...
		['v.EncodeNumber(enc).DecodeNumber(enc)', [{ v: 0, enc: 'uint8', result: 0 }, { v: 1055, enc: 'uint16', result: 1055 }, { v: 1055, enc: 'uint16le', result: 1055 }]],
		['v.EncodeString(enc).DecodeString(enc)', [{ v: '', enc: 'utf8', result: '' }, { v: '1055', enc: 'utf8', result: '1055' }, { v: '1055', enc: 'ucs2le', result: '1055' }]],
		['#ffff0001.Slice(1).FormatBuffer()', [{ result: 'ff0001' }]],
		['a.Byte(b).FormatBuffer()', [{ a: new Uint8Array([0xff, 0xff, 0x00, 0x01]).buffer, b: 3, result: '01' }, { a: new Uint8Array([0x10, 0x00]).buffer, b: 0, result: '10' }]],
		...
		['var s:number=0,Range(start, end).Transform(void(x:number){s=s+x}),s', [{ start: 1, end: 11, result: 55 }, { start: -1, end: -11, result: -65 }]],
		...
		['arr0.Transform( array(a:array){ a.Transform(number(b:number){b+12})} ).Flatten().Sum()', [{ arr0: [[1], [1, 2], [2, 3, 4]], result: 85 }]],
		...
		['str1.ParseJSON.prop1+str2.ParseJSON().prop2', [{ str1: '{"prop1":1}', str2: '{"prop2":20}', result: 21 }]],
		['str1.ParseJSON()+str2.ParseJSON()', [{ str1: '"p1"', str2: '"p2"', result: 'p1p2' }]],
		['obj1.FormatJSON()$obj2.FormatJSON()', [{ obj1: { p1: 'a' }, obj2: { p2: 'b' }, result: '{"p1":"a"}{"p2":"b"}' }]],
		['obj1.FormatJSON()', [{ obj1: { p1: 'a' }, result: '{"p1":"a"}' }]],
		...
		['var f=boolean(a:number){a=a*100, a>0},a.Filter(f).Sum()', [{ a: [-10, -20, 1, 2], result: 3 }]],
		['const f=??(){b = b + 1000, true}, a.Filter(f).Sum() + b', [{ a: [10, 20, 1, 2], b: 0, result: 4033 }]],
		...
		['Random.Buffer(c).Length + Random.String(c).Length', [{ c: 0, result: 0 }, { c: 64, result: 128 }]],
